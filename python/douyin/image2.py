...
def show_image(news_contents):
    ...
    header_font = ImageFont.truetype('Arial Unicode.ttf', 22)  # 标题的字体楷体，字号55
    draw.multiline_text(header_position, '互联网日报', '#726053', header_font)  # 入参分别是坐标，文字内容，文字色号，文字字体

    current_height = 100
    for line in news_wrap:
        if line.startswith('【'):
            news_font = ImageFont.truetype('Arial Unicode.ttf', 45)  # 标题的字体楷体，字号50
            draw.text((60, current_height + 30), line, '#726053', news_font)
            current_height += 80
        else:
            news_font = ImageFont.truetype('Arial Unicode.ttf', 55)  # 新闻字体30
